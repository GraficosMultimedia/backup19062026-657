...
if (!function_exists('company_profile')) {
    ...
    exit('El PDF no puede generarse: no se pudo cargar includes/company.php o company_profile().');
}

/* ===== RENDERER CORPORATIVO INLINE ===== */
...
$id=(int)($_GET['id'] ?? 0);
$order=order_get($id);
if(!$order){http_response_code(404);exit('Orden no encontrada.');}

$customer=[];
try{
    $st=db()->prepare('SELECT id,name,email,tax_number,phone,address,city,zip_code,state,country FROM cp_customers WHERE id=? LIMIT 1');
    $st->execute([(int)($order['customer_id'] ?? 0)]);
    $customer=$st->fetch() ?: [];
}catch(Throwable $e){$customer=[];}

$quote=quote_get((int)$order['quote_id']);
if(!$quote){$quote=[];}
$items=order_items($id);
$history=order_history($id);
$payments=finance_payment_list($id);
$finance=finance_order_summary($id);
$order['production_stage']=production_stage_label(production_get_stage($id));

$photoCount=0;
try{
    $st=db()->prepare('SELECT COUNT(*) FROM cp_order_photos WHERE order_id=?');
    $st->execute([$id]);
    $photoCount=(int)$st->fetchColumn();
}catch(Throwable $e){$photoCount=0;}

$company=company_profile();
$docroot=rtrim((string)($_SERVER['DOCUMENT_ROOT'] ?? __DIR__),'/');
$logoPath='';
$preferred=(string)($company['logo_pdf_path'] ?? '');
foreach([$preferred,'/assets/img/company/logo-pdf.jpg'] as $candidate){
    $candidate=trim($candidate);
    if($candidate==='')continue;
    $path=str_starts_with($candidate,'/')?$docroot.$candidate:$docroot.'/'.$candidate;
    if(is_file($path)){$logoPath=$path;break;}
}

try{
    $data=generate_order_pdf($order,$customer,$quote,$items,$finance,$payments,$history,$company,$logoPath,$photoCount);
}catch(Throwable $e){
    http_response_code(500);
    header('Content-Type: text/plain; charset=UTF-8');
    exit('Error al generar el PDF corporativo: ' . $e->getMessage());
}
header('Content-Type: application/pdf');
header('Content-Length: '.strlen($data));
$filename=preg_replace('/[^A-Za-z0-9._-]+/','_',(string)$order['order_number']) ?: 'orden-servicio';
header('Content-Disposition: inline; filename="'.$filename.'-corporativo.pdf"');
